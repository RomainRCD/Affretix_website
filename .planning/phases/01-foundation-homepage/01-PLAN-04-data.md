...
must_haves:
  truths:
    - "All 8 machine families are defined as typed TypeScript data with slug, name, description, icon, and machineCount"
    - "All 67+ machine types are defined with name, family slug, description, and available variants/capacities"
    - "The data layer exports are typed with exported TypeScript interfaces that other components can import"
    - "TypeScript compiles with 0 errors on the data files"
  artifacts:
    - path: "src/data/families.ts"
      provides: "8 machine family definitions with full descriptions, slugs, icons, machine counts"
      exports: ["MachineFamily", "machineFamilies"]
    - path: "src/data/machines.ts"
      provides: "67+ machine type definitions grouped by family slug"
      exports: ["Machine", "machines", "getMachinesByFamily"]
  key_links:
    - from: "src/data/machines.ts"
      to: "src/data/families.ts"
      via: "family slug field links machines to their family"
      pattern: "familySlug"
---

<objective>
Define the complete TypeScript data layer for all 8 machine families and their 67+ machine types. This is the single source of truth that Plans 02 and 03 read from, and that Phase 2 (Catalogue) will extend with images and richer data.

Purpose: Without typed data, the FamilyGrid in Plan 03 renders stub content. Without machine data, Phase 2 has no foundation to build catalogue pages on. Defining the full data layer in Phase 1 means Phase 2 can start immediately from a rich data baseline.

Output: `src/data/families.ts` (8 families, complete) and `src/data/machines.ts` (67+ machines, typed, grouped by family slug).
</objective>
...
<task type="auto">
  <name>Task 1: Write complete families.ts with 8 families</name>
  <files>src/data/families.ts</files>

  <read_first>
    - .planning/phases/01-foundation-homepage/01-RESEARCH.md — Pattern 4 (Machine Data Layer TypeScript), Architecture Patterns (data/families.ts shape)
    - src/data/families.ts (if it already exists from Plan 02 bootstrap — read and extend rather than overwrite)
  </read_first>

  <action>
If `src/data/families.ts` already exists (created as a stub by Plan 02), replace it entirely with the complete version below. If it does not exist, create it.

The 8 canonical families come from the project brief: Terrassement, Transport chantier, Voirie, Compactage, VRD/Réseaux, Manutention/Levage, Béton/Fondations, Démolition/Recyclage.
...
Note: `machineCount` values match the machine counts in `machines.ts` (Task 2). Update if the actual counts differ after Task 2 is complete.
  </action>

  <verify>
    <automated>cd /Users/romainrichard/PycharmProjects/Affretix_website && npx tsc --noEmit 2>&1 | grep "families" | head -10</automated>
  </verify>

  <acceptance_criteria>
    - `src/data/families.ts` exports `MachineFamily` type
    - `src/data/families.ts` exports `machineFamilies` array
    - `machineFamilies` has exactly 8 entries (grep: `{ slug:` should return 8 matches)
    - All 8 slugs present: terrassement, transport-chantier, voirie, compactage, vrd-reseaux, manutention-levage, beton-fondations, demolition-recyclage
    - Each entry has non-empty `name`, `description`, `icon`, `machineCount > 0`
    - `npx tsc --noEmit` shows 0 errors related to families.ts
  </acceptance_criteria>

  <done>families.ts written with all 8 complete family definitions including full descriptions.</done>
</task>

<task type="auto">
  <name>Task 2: Write machines.ts with all machine types grouped by family</name>
  <files>src/data/machines.ts</files>

  <read_first>
    - src/data/families.ts (read to confirm exact slugs before writing machine data)
    - .planning/phases/01-foundation-homepage/01-RESEARCH.md — Pattern 4 (Machine Data Layer), Architecture Patterns
  </read_first>

  <action>
Create `src/data/machines.ts` with the full machine catalogue. The data comes from the project brief: 67+ types across 8 families, all avec opérateur.

Machine type: name, familySlug (matches families.ts slug), description, available variants (size/tonnage options), and optionally accessories.
...
export const machines: Machine[] = [
  ...
  {
    slug: 'porteur-polyvalent',
    name: 'Porteur polyvalent',
    familySlug: 'transport-chantier',
    description: 'Porteur à benne amovible (ampliroll) pour transport de matériaux en bennes interchangeables.',
    variants: [{ label: '20m³ — 26t' }],
  },
...
```
  ...
  <verify>
    <automated>cd /Users/romainrichard/PycharmProjects/Affretix_website && npx tsc --noEmit 2>&1 | tail -10 && node -e "const {machines} = require('./src/data/machines.ts'); console.log('Machine count:', machines.length)" 2>/dev/null || npx ts-node --skip-project -e "import {machines} from './src/data/machines'; console.log(machines.length)" 2>/dev/null || echo "TS node not available — check tsc instead"</automated>
  </verify>

  <acceptance_criteria>
    - `src/data/machines.ts` exports `Machine` type, `MachineVariant` type, `machines` array, and `getMachinesByFamily` function
    - `machines` array has >= 67 entries
    - Every entry has a non-empty `slug`, `name`, `familySlug`, `description`, `variants` array with >= 1 entry
    - All 8 family slugs are represented: grep for each slug (terrassement, transport-chantier, voirie, compactage, vrd-reseaux, manutention-levage, beton-fondations, demolition-recyclage) returns >= 1 match
    - `getMachinesByFamily` function exported and callable
    - `npx tsc --noEmit` exits 0
  </acceptance_criteria>

  <done>machines.ts written with 67+ machine types across 8 families. TypeScript compiles clean. getMachinesByFamily helper exported for Phase 2 use.</done>
</task>
...
|----------|-------------|
| Static data | machines.ts and families.ts are compiled into the bundle — no runtime external data source |

## STRIDE Threat Register

| Threat ID | Category | Component | Disposition | Mitigation Plan |
|-----------|----------|-----------|-------------|-----------------|
| T-04-01 | Tampering | Static data files | accept | TypeScript type system enforces data shape at compile time. No runtime mutation. Threat surface is git repository access, not runtime. |
</threat_model>
...
grep -c "familySlug:" src/data/machines.ts   # must be >= 67

# Verify all 8 family slugs present
grep -c "terrassement" src/data/machines.ts
grep -c "transport-chantier" src/data/machines.ts
grep -c "voirie" src/data/machines.ts
grep -c "compactage" src/data/machines.ts
grep -c "vrd-reseaux" src/data/machines.ts
grep -c "manutention-levage" src/data/machines.ts
grep -c "beton-fondations" src/data/machines.ts
grep -c "demolition-recyclage" src/data/machines.ts

# Verify exports
grep "export.*getMachinesByFamily" src/data/machines.ts   # must match
grep "export.*machines:" src/data/machines.ts             # must match
grep "export type Machine" src/data/machines.ts           # must match
```
</verification>

<success_criteria>
- `src/data/families.ts`: 8 families with complete descriptions, all slugs matching the 8 canonical families
- `src/data/machines.ts`: 67+ machine types, all typed with `Machine` interface, all with variants
- `getMachinesByFamily` function callable and filters correctly by slug
- `npx tsc --noEmit` exits 0 on both files
- `machineCount` values in families.ts match actual machine counts per family in machines.ts
</success_criteria>

<output>
After completion, create `.planning/phases/01-foundation-homepage/01-04-SUMMARY.md` with:
- What was built (complete data layer for 8 families and 67+ machines)
- Exact machine count per family
- Any adjustments to machineCount values in families.ts
