...
// Champs requis (chaîne non vide)
const REQUIRED = ['nom', 'telephone', 'email'] as const
...
function buildEmailHtml(data: ContactPayload): string {
  ...
  const chantier =
    [data.adresseChantier, [data.cpChantier, data.villeChantier].filter(Boolean).join(' ')]
      .filter((s) => s && s.trim())
      .join(', ') || undefined
  ...
  <div style="max-width:600px;margin:0 auto;background:#fff;border:1px solid #e0e0e0;">
    ...
    <div style="padding:24px 32px;">
      <table style="border-collapse:collapse;width:100%;">
        <tbody>
          ${row('Matériel', data.materiel)}
          ${row('Entreprise', data.entreprise)}
          ${row('SIRET', data.siret)}
          ${row('Nom', [data.prenom, data.nom].filter(Boolean).join(' '))}
          ${row('Téléphone', data.telephone)}
          ${row('Email', data.email)}
          ${row('Période', periode)}
          ${row('Chantier', chantier)}
          ${row('Commentaire', data.commentaire?.replace(/\n/g, '<br />'))}
        </tbody>
      </table>
    </div>
...
export async function POST(request: NextRequest) {
  ...
  try {
    const contactName = [body.prenom, body.nom].filter(Boolean).join(' ') || body.nom
    const { error } = await resend.emails.send({
      // Adresse de test Resend tant que le domaine affretix.fr n'est pas vérifié
      from: 'Affretix Devis <onboarding@resend.dev>',
      to: ['contact@affretix.fr'],
      replyTo: body.email,
      subject: `Demande de devis — ${body.materiel || contactName}${body.entreprise ? ` (${body.entreprise})` : ''}`,
